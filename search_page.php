<?php

include 'config.php';

session_start();

$user_id = $_SESSION['user_id'];

if(!isset($user_id)){
   header('location:login.php');
};

if(isset($_POST['add_to_cart'])){

   $product_name = $_POST['product_name'];
   $product_price = $_POST['product_price'];
   $product_image = $_POST['product_image'];
   $product_quantity = $_POST['product_quantity'];

   $check_cart_numbers = mysqli_query($conn, "SELECT * FROM cart WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');

   if(mysqli_num_rows($check_cart_numbers) > 0){
      $message[] = 'Produk Sudah Ada Dalam Keranjang!';
   }else{
      mysqli_query($conn, "INSERT INTO cart(user_id, name, price, quantity, image) VALUES('$user_id', '$product_name', '$product_price', '$product_quantity', '$product_image')") or die('query failed');
      $message[] = 'Produk Ditambahkan Ke Keranjang!';
   }

};

?>

<!DOCTYPE html>
<html lang="en">
<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>Pencarian</title>
   
   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">

   <link rel="stylesheet" href="css/style.css">

   <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>

   <script>
      $(document).ready(function() {
         function updateSearchResults(searchQuery) {
            if (searchQuery !== '') {
               $.post("live_search.php", { search: searchQuery }, function(data) {
                  $("#searchResults").html(data);

                  if (data.trim() !== '') {
                     $("#emptyMessage").html('');
                  } else {
                     $("#emptyMessage").html('');
                  }
               });
            } else {
               $("#searchResults").html('');
               $("#emptyMessage").html('');
            }
         }

         updateSearchResults($("#searchInput").val());

         $("#searchInput").on("input", function() {
            var searchQuery = $(this).val();
            updateSearchResults(searchQuery);
         });

         $("#searchForm").submit(function(event) {
            var searchQuery = $("#searchInput").val();

            updateSearchResults(searchQuery);
         });
      });
   </script>
</head>
<body>
   
<?php include 'header.php'; ?>

<div class="heading">
   <h3>Mau Cari Apa?</h3>
   <p> <a href="home.php">Home</a> / Pencarian </p>
</div>

<section class="search-form">
   <form action="" method="post" id="searchForm">
      <input type="text" name="search" id="searchInput" placeholder="Cari Produk" class="box">
   </form>
</section>

<section class="products" style="padding-top: 0;">
   <div id="searchResults"></div>
   <div class="box-container">
      <?php
      $search_item = isset($_POST['search']) ? $_POST['search'] : '';
      if ($search_item !== '') {
         $select_products = mysqli_query($conn, "SELECT * FROM products WHERE name LIKE '%$search_item%'") or die('query failed');
         if (mysqli_num_rows($select_products) > 0) {
            while ($fetch_product = mysqli_fetch_assoc($select_products)) {
               ?>
               <form action="" method="post" class="box">
                  <img src="uploaded_img/<?php echo $fetch_product['image']; ?>" alt="" class="image">
                  <div class="name"><?php echo $fetch_product['name']; ?></div>
                  <div class="price">Rp<?php echo number_format($fetch_product['price'], 0, ',', '.'); ?></div>
                  <input type="number" class="qty" name="product_quantity" min="1" value="1">
                  <input type="hidden" name="product_name" value="<?php echo $fetch_product['name']; ?>">
                  <input type="hidden" name="product_price" value="<?php echo $fetch_product['price']; ?>">
                  <input type="hidden" name="product_image" value="<?php echo $fetch_product['image']; ?>">
                  <a href="detail_produk_user.php?id=<?php echo $fetch_product['id']; ?>" class="detail-btn" target="_blank">Detail</a>
                  <input type="submit" class="btn" value="Tambah Ke Keranjang" name="add_to_cart">
               </form>
               <?php
            }
         }
      }
      ?>
   </div>
</section>

<?php include 'footer.php'; ?>

<script src="js/script.js"></script>

</body>
</html>
